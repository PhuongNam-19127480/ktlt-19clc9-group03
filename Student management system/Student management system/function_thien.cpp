...
void LoadStudent(ifstream& finput, LinkedList& Stu) {
	...
	if (!finput.is_open()) {
		...
		getline(finput, Year);
		Stu.count++;
		Node* p = CreateNodeThien(No, ID, Lastname, Firstname, Gender, Day, Month, Year);
		Stu.head = p;
...
void Delete_Student_From_Class(LinkedList& Class, LinkedList& TempList) {
	...
	while (i < n) {
		...
		getline(cin, StuID);
		if (Class.head->Info.ID == StuID) {
			Node* temp = CreateNodeThien(Class.head->Info.No, Class.head->Info.ID, Class.head->Info.Lastname, Class.head->Info.Firstname, Class.head->Info.Gender, Class.head->Info.Day, Class.head->Info.Month, Class.head->Info.Year);
			...
			while (current != Class.tail) {
				if (current->Info.ID == StuID) {
					Node* temp = CreateNodeThien(current->Info.No, current->Info.ID, current->Info.Lastname, current->Info.Firstname, current->Info.Gender, current->Info.Day, current->Info.Month, current->Info.Year);
					temp->next = TempList.head;
					TempList.head = temp;
					Node* p = current;
					previous->next = current->next;
